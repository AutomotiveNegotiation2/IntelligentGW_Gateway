...
{   
    ...
    if(Using_Size > 0){
        ...
        memset(Used_Info.Memroy_Full_Start, 0x00, Used_Info.Memroy_Full_Size);
...
/* 
    @brief 
    할당된 메모리의 데이터 저장영역 전체를 0x00으로 셋 
    Data_Info:데이터 저장 영역의 정보
*/
void F_i_Memory_Data_Flush(struct Memory_Used_Data_Info_t *Data_Info)
{   
    pthread_mutex_lock(&Data_Info->mtx);
    memcpy(Data_Info->p_start, 0x00, Data_Info->p_end - Data_Info->p_start);
    for(int i = 0; i < *Data_Info->Data_Count; i++)
    { 
        Data_Info->Data_Pointer_List[i] = Data_Info->p_now;
        Data_Info->Data_Pointer_List[i] = NULL;
        Data_Info->Data_Size_List[i] = 0;
    }
    *Data_Info->Data_Count = 0;
    Data_Info->p_now = Data_Info->p_start;
    pthread_mutex_unlock(&Data_Info->mtx);
