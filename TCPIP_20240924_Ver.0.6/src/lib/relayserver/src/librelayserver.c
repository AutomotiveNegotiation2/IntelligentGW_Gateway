...
struct socket_info_t F_s_RelayServer_TcpIp_Initial_Server(char *Device_Name, int *Port, int *err)
{
    ...
    if(!Port)
    {
        F_RealyServer_Print_Debug(2, "[Error][%s] No Input Argurements.(Port:%p)\n", __func__, Port);
        *err = -1;
        return Socket_Info;
    }else{
        ...
        if(Device_Name)
        {
            ...
            if(ret < 0)
            {
                F_RealyServer_Print_Debug(2,"[Error][%s] Return_Value:%d\n", __func__, ret);
                *err = -1;
                return Socket_Info;
            }
...
int F_i_RelayServer_TcpIp_Get_Address(char *Device_Name, char Output_IPv4Adrress[40])
{
    ...
    if(!Device_Name)
    {
        F_RealyServer_Print_Debug(2, "[Error][%s] No Input Argurements.(Device_Name:%p)\n", __func__, Device_Name);
        return -1;
    }
...
int F_i_RelayServer_TcpIp_Task_Run(struct socket_info_t *Socket_Info)
{
    ...
    if(ret < 0)
    {
            F_RealyServer_Print_Debug(2, "[Error][%s][f_i_RelayServer_TcpIp_Bind] Return Value:%d", __func__, ret);
    }else{
                    ...
                    if(out_data)
                    {
                        ...
                        { 
                            for(int client_is = 0; client_is < MAX_CLIENT_SIZE; client_is++)
                            {
                                ...
                                {
                                    F_RealyServer_Print_Debug(2,"[Info][%s] Now_Job:%d\n", __func__, Data_Header_Info.Job_State);
                                    ...
                                    switch(Now_Job)
                                    {
                                        ...
                                        case HandOverReminingData:
                                            ...
                                            F_RealyServer_Print_Debug(2,"[Debug][%s][%d][Push:%s/%d]\n", __func__, __LINE__, out_data, data_size);
...
void* th_RelayServer_TcpIp_Task_Server(void *socket_info)
{
    ...
    if(ret == -1)
    {
        F_RealyServer_Print_Debug(2,"[Error][%s][listen] Return Value:%d\n", __func__, ret);
        return NULL;
    }
#endif
    ...
    while(1)
    {
        ...
        if(str_len > 0)
        {
            ...
            {
                F_RealyServer_Print_Debug(2,"[Error][%s][%d] Connected Client Num > MAX_CLIENT_SIZE:%d/%d\n", __func__, __LINE__, *G_Clients_Info.connected_client_num, MAX_CLIENT_SIZE);
            }else{
                for(client_is = 0; client_is < MAX_CLIENT_SIZE; client_is++)
                {
                    ...
                    }else if(G_Clients_Info.socket[client_is] == (int)(from_adr.sin_addr.s_addr))
                    {
                        ...
                        //F_RealyServer_Print_Debug(2,"[DEBUG][%s][%d] Connected Client %s:%d\n", __func__, __LINE__, addr_str, from_adr.sin_port);
...
int f_i_RelayServer_TcpIp_Bind(int *Server_Socket, struct sockaddr_in Socket_Addr)
{
    ...
    do
    {
        ...
        if(ret < 0 ) 
        {
            F_RealyServer_Print_Debug(2, "[Error][%s][Return_Value:%d]", __func__, ret);
...
int f_i_RelayServer_TcpIp_Setup_Socket(int *Socket, int Timer, bool Linger)
{
    if(!Socket || Timer < 0)
    {
        F_RealyServer_Print_Debug(2, "[Error][%s][No Input Argurements.](Socket:%p, Timer:%d)\n", __func__, Socket, Timer);
        return -1;
    }
    if(Linger)
    {
...
int f_i_RelayServer_Job_Process_Finish(struct data_header_info_t *Now_Header, uint8_t *Data, int Client_is)
{
    if(Data)
    {
        ...
        F_RealyServer_Print_Debug(2, "[Error][%s][No Data]\n", __func__);
        return -1;
    }
    return 0;
}
            ...
            switch(Now_Header->Job_State)
            {
                case FirmwareInfoReport:
                    if(Now_Header->Message_size == 23 && Payload[Now_Header->Message_size - 1] == 0xAA)
                    {
                        ...
                        F_RealyServer_Print_Debug(2, "[Error][%s][Now_Header->Message_size:%d, ETX:%02X]\n",__func__, Now_Header->Message_size, Payload[Now_Header->Message_size - 1]);
                        return -3;
                    }
                    break;
                case ProgramInfoReport:
    ...
    if(Data)
    {
        ...
        switch(Now_Header->Job_State)
        {
            ...
            case ProgramInfoRequest:
                ...
                break;
            default:
                F_RealyServer_Print_Debug(2, "[Error][%s][Job_State:%d]\n", __func__, Now_Header->Job_State);
                return -1;
        }   
...
//* ADD 230906
        ...
        if(Now_Header->Message_size > 0)
        {
            ...
            int ret = f_i_RelayServer_HTTP_Check_URL((const char*)Payload);
            if(ret == 0)
            {   
                F_RealyServer_Print_Debug(2, "[Error][%s][Invaliad URL:%s]\n",__func__, Payload);
                return -1;
            }
            ...
            curl_easy_setopt(curl_handle, CURLOPT_URL, Payload);
            ...
            switch(Now_Header->Job_State)
            {
                ...
                case ProgramInfoIndication:
                    if(Now_Header->Message_size <= 0)
                    {
                        ...
                        if(ret <= 0)
                        {
                            F_RealyServer_Print_Debug(0,"[Debug][%s][send:%d, ret:%p]\n", __func__, __LINE__, ret);_DEBUG_LOG
                        }else{
                            printf("send:%d\n", ret);_DEBUG_LOG
                        }
                        break;
                        ...
                        uint32_t total_div_num = (send_info.total_data_len / send_info.div_len ) + ((send_info.total_data_len % send_info.div_len  == 0) ? 0 : 1);
                        ...
                        while(send_p_n < Now_Header->Message_size)
                        {
                            ...
                            if(Now_Header->Message_size - send_p_n >= 1024)
                            {
                                ...
                                printf("Now_Header->Message_size - send_p_n:%d - %d = %d\n", Now_Header->Message_size, send_p_n, Now_Header->Message_size - send_p_n);
                                ...
                                memcpy(sendbuf, &send_info, div_hdr_len);
                                printf("Now_Header->Message_size - (send_p_n + left_len):%d - (%d + %d) = %d\n", Now_Header->Message_size, send_p_n, left_len, Now_Header->Message_size - (send_p_n + left_len));
                                printf("LEFT_LEN:%d\n", left_len);                      
                            ...
                            if(ret <= 0)
                            {
                                F_RealyServer_Print_Debug(0,"[Debug][%s][%d][send:%d/%d]\n", __func__, __LINE__, ret, Now_Header->Message_size);
                            }else{
                        ...
                        #if 0 
                        uint8_t *out_data = malloc(sizeof(uint8_t) * HEADER_SIZE + 20);_DEBUG_LOG
                        F_RealyServer_Print_Debug(4,"[Debug][%s][malloc:%d, Address:%p]\n", __func__, __LINE__, out_data);
                    ...
                    break;
                default:
                    F_RealyServer_Print_Debug(0, "[Error][%s][Job_State:%d]", __func__, Now_Header->Job_State);
                    return -1;
            }
            close(sock);
        }else{
...
int f_i_RelayServer_HTTP_Task_Run(struct data_header_info_t *Now_Header, struct http_socket_info_t *http_socket_info, uint8_t **out_data)
{
    ...
    if(res != CURLE_OK) {
        F_RealyServer_Print_Debug(2, "[Error][%s]: %s\n", __func__, curl_easy_strerror(res));
        return -1;
    }
   ...
   do 
    {
        ...
        do {
            ...
            {
                F_RealyServer_Print_Debug(2, "[Error][%s]: timeout.\n", __func__);
                return -1;
            }
...
No_GW_SLEEP_CONNECTIONING_NUVO: 
                if(nubo_info->state != GW_RECEIVE_DRIVING_HISTORY_DATA_FROM_NOVO)
                {
                    ...
                    switch(recv_buf[9])
                    {
                        ...
                        case NUVO_SIGNAL_STATE_DOWNLOAD_PREPARE:
                        {
                            ...
                            printf("[DRIVING HISTORY] [Waiting Driving History Data Info] Prepare File Size %d ...... %ld[s]\n", file_data_len, time(NULL) - now);
                ...
                switch(nubo_info->state)
                {
                    ...
                    case GW_RECEIVE_DRIVING_HISTORY_DATA_FROM_NOVO:
                    {
                        ...
                        while(1)
                        {
                            ...
                            if(recv_len > 0)
                            {
                                ...
                                if(time(NULL) - recv_end_time > 3)
                                {
                                    ...
                                    {
                                        printf("[DRIVING HISTORY] [Recvive Driving History Data] Finish Recvive From NUVO, Recvive Data Length : %d/%d ...... %d[s]\n", total_recv_len, file_data_len, time(NULL) - now);
                                    }
                                    break;
                                }
                            }
                        ...
                        if(file_path)Relay_safefree(file_path);
                        if(recv_file_data)Relay_safefree(recv_file_data);
                        fclose(fp);
...
GW_JOB_BY_NUBO_DONE:
    ...
    json_value_Relay_safefree(rootValue);
