    ...
    if(!Port)
    {
        ...
        *err = -1;
        ...
        Socket_Info.Port = *Port;DEBUG_1
         //Socket_Setup
        Socket_Info.Socket = socket(PF_INET, SOCK_DGRAM, 0);DEBUG_1
        memset(&Socket_Info.Socket_Addr, 0x00, sizeof(Socket_Info.Socket_Addr)); DEBUG_1
        Socket_Info.Socket_Addr.sin_family = AF_INET;DEBUG_1
...
void* th_RelayServer_TcpIp_Task_Server(void *socket_info)
{
    ...
    socklen_t adr_sz = sizeof(Client_Address);DEBUG_1
#if 0 
            ...
            if(*G_Clients_Info.connected_client_num == MAX_CLIENT_SIZE)
            {
                ...
                for(client_is = 0; client_is < MAX_CLIENT_SIZE; client_is++)
                {
                    ...
                    }else if(G_Clients_Info.socket[client_is] == (int)(from_adr.sin_addr.s_addr))
                    {
                        ...
                        //F_RealyServer_Print_Debug(0,"[DEBUG][%s][%d] Connected Client %s:%d\n", __func__, __LINE__, addr_str, from_adr.sin_port);
...
struct client_data_info_t f_s_RelayServer_Job_Process_Initial(struct data_header_info_t *Now_Header, uint8_t *Data, int *err)
{
    ...
    if(Data)
    {
        ...
        if(Payload[0] == 0x44) // Check STX
        {
            ...
            {
                case 1:
                    if(Now_Header->Message_size  + 1 > 24) //Will Make the Over Recv Error Solution
                    {
...
int f_i_RelayServer_Job_Process_InfoReport(struct data_header_info_t *Now_Header, uint8_t *Data)
{
    if(Data)
    {
        ...
        if(Payload[0] == 0x44) // Check STX
        {
            ...
            {
                case FirmwareInfoReport:
                    if(Now_Header->Message_size + 1 == 24 && Payload[Now_Header->Message_size] == 0xAA)
                    {
                        Now_Header->Job_State = 3;
                        Data[0] = *"3";
                        F_RealyServer_Print_Debug(2, "[Info][%s][Job_State:%d, STX:%02X ETX:%02X]\n",__func__, Now_Header->Job_State, Payload[0], Payload[Now_Header->Message_size]);
                        return Now_Header->Job_State;
                    }else{
                        F_RealyServer_Print_Debug(0, "[Error][%s][Now_Header->Message_size:%d, ETX:%02X]\n",__func__, Now_Header->Message_size, Payload[Now_Header->Message_size]);
                        return -3;
                    }
                    break;
                case ProgramInfoReport:
...
int f_i_RelayServer_Job_Process_InfoRequest(struct data_header_info_t *Now_Header, uint8_t **Data, struct Memory_Used_Data_Info_t *Data_Info)
{
    if(Data)
    {
        ...
        switch(Now_Header->Job_State)
        {
            ...
            http_socket_info->Data_Info = Data_Info;DEBUG_1
...
int f_i_RelayServer_HTTP_Task_Run(struct curl_info_t *curl_info)
{
    ...
    CURLcode res;
    char *URL;
    switch(curl_info->Now_Header->Job_State)
    {
        case FirmwareInfoRequest:
    ...
    return curl_info->Now_Header->Job_State;
}
...
void *th_RelayServer_HTTP_Task_Receive(void *data)
{
    ...
    if(buf_len > 0)
    {
		...
		printf("\n");
#endif
        struct data_header_info_t *Now_Header = curl_info->Now_Header;
        switch(Now_Header->Job_State)
        {
            case FirmwareInfoRequest:
        ...
        Relay_safefree(Http_Recv_data);
    }
    /* always cleanup */
    memset(buf, 0x00, sizeof(buf));
    curl_easy_cleanup(curl_info->curl);
    F_RealyServer_Print_Debug(0"[Debug][%s][Free:%d, Address:%p]\n", __func__, __LINE__, curl_info->request);
    //Relay_safefree(curl_info->request);
    F_RealyServer_Print_Debug(0,"[Debug][%s][Free:%d, Address:%p]\n", __func__, __LINE__, curl_info);
    Relay_safefree(curl_info);
    return NULL;
}
#else // 20240909 Change
...
#if 1
        printf("RECV_HTTP_REQ:%d\n",  http_socket_info->request_len);
        for(int i = 0;  i < http_socket_info->request_len; i++)
        {
            printf("%c", http_socket_info->request[i]);
        }
        printf("\n");
#endif
...
No_GW_SLEEP_CONNECTIONING_NUVO: 
                ...
                switch(nubo_info->state)
                {
                    ...
                    case GW_RECEIVE_DRIVING_HISTORY_DATA_FROM_NOVO:
                    {
                        ...
                        if(file_path)Relay_safefree(file_path);DEBUG_1
                        if(recv_file_data)Relay_safefree(recv_file_data);DEBUG_1
                        fclose(fp);
